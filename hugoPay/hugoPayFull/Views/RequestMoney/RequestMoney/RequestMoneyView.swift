...
class RequestMoneyView: UIView {
    ...
    var requestMoneyConfirmation: ((_ recipientType: RecipientType,
                                    _ areacode: String,
                                    _ recipient: String,
                                    _ requestAmount: Double,
                                    _ contact: HugoPayContact?) -> ())?
    
    lazy var viewModel: RequestMoneyViewModel = {
    ...
    func prepareViews(_ vc_view: PullUpView)  {
        let nib = Bundle.main.loadNibNamed("RequestMoneyView", owner: self, options: nil)
    ...
    @IBAction func didTapNextButton(_ sender: Any) {
        view.endEditing(true)
        guard let requestAmount = amountLbl.text,
        ...
        if viewModel.verifyRequestMoneyInfo(with: requestAmount) {
            self.removeObservers()
            let requestAmountAsDouble = Double(requestAmount.removeFormatAmount())
            requestMoneyConfirmation?(recipientType, areaCode, recipient, requestAmountAsDouble, phoneView.selectedContact ?? nil)
        } else {
            generator.notificationOccurred(.error)
