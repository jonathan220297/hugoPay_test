...
open class PinInputView: UIView {
    ...
    /**
     The color of the line under each digit
     */
    open var backgroundLabelColor = UIColor.clear {
    ...
    open weak var delegate: PinInputViewDelegate?
    ...
    override open func layoutSubviews() {
        ...
        var characterWidth = frame.height / 6
        var characterHeight = frame.height / 4
        ...
        for (index, label) in labels.enumerated() {
            ...
            circles[index].frame = CGRect(x: x, y: characterWidth*0.25, width: characterWidth, height: characterWidth)
            circles[index].cornerRadius = characterWidth/2
    ...
    public func cleanLabels(){
        ...
        for circle in circles{
            ...
            circle.borderColor = UIColor.init(hexString: "bfb4db")
    ...
    fileprivate func setup() {
        ...
        for i in 0..<numberOfDigits {
            ...
            label.backgroundColor = backgroundLabelColor
    ...
    public func viewBecomeFirstResponder() {
        textField!.becomeFirstResponder()
    }
    ...
    fileprivate func didChange(_ backspaced: Bool = false) {
        ...
        if lastIndex >= 0 {
            ...
            circles[lastIndex].backgroundColor = UIColor.init(hexString: "3b1169")
            circles[lastIndex].borderColor = .clear
            circles[lastIndex].borderWidth = 0.0
        }
        
        
        
        delegate?.pinDidChange(pinInputView: self)
    }
...
extension PinInputView: UITextFieldDelegate {
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if  let tx =  textField.text, !tx.isEmpty ,isBackSpace == -92 {
            textField.text?.removeLast()
            
            circles[textField.text?.count ?? 0].backgroundColor = .clear
            circles[textField.text?.count ?? 0].borderColor = UIColor.init(hexString: "bfb4db")
            circles[textField.text?.count ?? 0].borderWidth = 1.0
            didChange(true)
            return false
        }
